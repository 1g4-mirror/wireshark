...
wspstat_init(const char *opt_arg, void *userdata _U_)
{
	...
	wsp_vals_status_p = VALUE_STRING_EXT_VS_P(vs_get_external_value_string_ext("wsp_vals_status_ext"));
	...
	{
		sp->pdu_stats[i].packets = 0;
		sp->pdu_stats[i].type = try_val_to_str_ext( index2pdut( i ), vs_get_external_value_string_ext("wsp_vals_pdu_type_ext"));
	}
