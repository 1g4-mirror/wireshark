...
rtp_packet(void *tap_offset_ptr, packet_info *pinfo, epan_dissect_t *edt, void const *rtp_info_ptr, tap_flags_t flags)
{
    ...
    if (strinfo==NULL) {
        ...
        if (!strinfo->first_payload_type_name) {
            strinfo->first_payload_type_name = val_to_str_ext(strinfo->first_payload_type, vs_get_external_value_string_ext("rtp_payload_type_short_vals_ext"), "%u");
        }
        ...
        if(gai != NULL) {
            ...
            if (gai != NULL) {
                ...
                new_gai->frame_label = ws_strdup_printf("%s (%s) %s%s%s",
                        ...
                        "":val_to_str_ext_const(rtp_listinfo->rtp_event, vs_get_external_value_string_ext("rtp_event_type_values_ext"), "Unknown RTP Event"),
        ...
        switch(t38_info->Data_Field_field_type_value) {
            ...
            case 4: /* hdlc-fcs-OK-sig-end */
                tmp_str1 = val_to_str_ext_wmem(NULL, t38_info->t30_Facsimile_Control & 0x7F,
                            vs_get_external_value_string_ext("t30_facsimile_control_field_vals_short_ext"),
                            "Ukn (0x%02X)");
                ...
                tmp_str1 = val_to_str_ext_wmem(NULL, t38_info->t30_Facsimile_Control & 0x7F,
                            vs_get_external_value_string_ext("t30_facsimile_control_field_vals_ext"),
                            "Ukn (0x%02X)");
...
isup_calls_packet(void *tap_offset_ptr, packet_info *pinfo, epan_dissect_t *edt, const void *isup_info, tap_flags_t flags _U_)
{
    ...
    if (callsinfo!=NULL) {
        ...
        frame_label = g_strdup(val_to_str_ext_const(pi->message_type, vs_get_external_value_string_ext("isup_message_type_value_acro_ext"), "Unknown"));
        ...
        switch(pi->message_type) {
            ...
            case 12: /* RELEASE */
                ...
                comment = ws_strdup_printf("Cause %i - %s",
                        pi->cause_value,
                        val_to_str_ext_const(pi->cause_value, vs_get_external_value_string_ext("q931_cause_code_vals_ext"), "(Unknown)"));
                break;
        }
            ...
            if (tapinfo->h225_cstype == H225_SETUP) {
                ...
                if (tapinfo->q931_cause_value != 0xFF) {
                    comment = ws_strdup_printf("H225 Q931 Rel Cause (%i):%s", tapinfo->q931_cause_value,
                            val_to_str_ext_const(tapinfo->q931_cause_value, vs_get_external_value_string_ext("q931_cause_code_vals_ext"), "<unknown>"));
                } else { /* Cause not set */
