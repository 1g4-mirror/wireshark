...
tap_packet_status BluetoothDevicesDialog::tapPacket(void *tapinfo_ptr, packet_info *pinfo, epan_dissect_t *, const void *data, tap_flags_t)
{
    ...
    if (tap_device->type == BLUETOOTH_DEVICE_LOCAL_VERSION) {
        ...
        item->setText(column_number_manufacturer,   val_to_str_ext_const(tap_device->data.local_version.manufacturer, vs_get_external_value_string_ext("bluetooth_company_id_vals_ext"), "Unknown 0x%04x"));
    }
    if (tap_device->type == BLUETOOTH_DEVICE_REMOTE_VERSION) {
        ...
        item->setText(column_number_manufacturer,   val_to_str_ext_const(tap_device->data.remote_version.manufacturer, vs_get_external_value_string_ext("bluetooth_company_id_vals_ext"), "Unknown 0x%04x"));
    }
