...
int dissect_ilnp(tvbuff_t* tvb, packet_info* pinfo, proto_tree* tree, void* data _U_)
{
    ...
    alloc_address_wmem(pinfo->pool, &pinfo->src, AT_ILNP_NID, 8, pinfo->net_src.data);
    alloc_address_wmem(pinfo->pool, &pinfo->dst, AT_ILNP_NID, 8, pinfo->net_dst.data);
    ...
    alloc_address_wmem(pinfo->pool, &ilnph->ilnp_src_nid, AT_ILNP_NID, 8, pinfo->net_src.data);
    alloc_address_wmem(pinfo->pool, &ilnph->ilnp_dst_l64, AT_ILNP_L64, 8, pinfo->net_dst.data);
    alloc_address_wmem(pinfo->pool, &ilnph->ilnp_dst_nid, AT_ILNP_NID, 8, pinfo->net_dst.data);
