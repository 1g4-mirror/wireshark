...
static int hf_bist_occured_cross;
...
dissect_bist_itch(tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree, void *data _U_)
{
    ...
    ti = proto_tree_add_protocol_format(tree, proto_bist, tvb, 0, -1, "bist ITCH, %s", type_desc);
    ...
    case 'R': {
        proto_tree_add_item(bist_tree, hf_bist_nanosecond, tvb, offset, 4, ENC_BIG_ENDIAN);
        offset += 4;
    ...
    case 'L': {
        proto_tree_add_item(bist_tree, hf_bist_nanosecond, tvb, offset, 4, ENC_BIG_ENDIAN); offset += 4;
        proto_tree_add_item(bist_tree, hf_bist_orderbook_id, tvb, offset, 4, ENC_BIG_ENDIAN); offset += 4;
    ...
    case 'A': {
        ...
        guint64 oid;
        proto_tree_add_item_ret_uint64(bist_tree, hf_bist_order_id, tvb, offset, 8, ENC_BIG_ENDIAN, &oid);
        offset += 8;
        col_append_fstr(pinfo->cinfo, COL_INFO, "%" PRIu64 " ", oid);

        proto_tree_add_item(bist_tree, hf_bist_orderbook_id, tvb, offset, 4, ENC_BIG_ENDIAN); offset += 4;
        proto_tree_add_item(bist_tree, hf_bist_side, tvb, offset, 1, ENC_NA); offset += 1;
        ...
        NEED(8);
        guint64 q;
        proto_tree_add_item_ret_uint64(bist_tree, hf_bist_quantity, tvb, offset, 8, ENC_BIG_ENDIAN, &q);
        offset += 8;
    ...
    case 'C': {
        ...
        proto_tree_add_item(bist_tree, hf_bist_occured_cross, tvb, offset, 1, ENC_ASCII); offset += 1;
    ...
    case 'M': {
        ...
        proto_tree_add_item(bist_tree, hf_bist_leg_side, tvb, offset, 1, ENC_NA); offset += 1;
...
void proto_register_bist(void)
{
    static hf_register_info hf_bist[] = {
        ...
        { &hf_bist_occured_cross,     { "Occurred at Cross",       "bist-itch.occured_cross",           FT_STRING, BASE_NONE, NULL, 0x0, NULL, HFILL } },
        ...
        { &hf_bist_price_decimals,    { "Price Decimals",          "bist-itch.price_decimals",          FT_UINT8,  BASE_DEC,  NULL, 0x0, NULL, HFILL } },
        { &hf_bist_nominal_decimals,  { "Nominal Decimals",        "bist-itch.nominal_decimals",        FT_UINT8,  BASE_DEC,  NULL, 0x0, NULL, HFILL } },
        ...
        { &hf_bist_strike_price_decimals,{ "Strike Price Decimals","bist-itch.strike_price_decimals",   FT_UINT8,  BASE_DEC,  NULL, 0x0, NULL, HFILL } },
