...
static DNS_SVCB_PARAMS_COMMON_LIST_T(dissect_dns_svcb_params_hf);
 ...
 */
const value_string dns_svcb_param_key_vals[] = {
                        ...
                        proto_tree *tree, int offset, int offset_end)
{
  ...
  while (cur_offset < offset_end) {
    svcb_param_ti = proto_tree_add_item(tree, hf->hf.dns_svcb_param, tvb, cur_offset, -1, ENC_NA);
    svcb_param_tree = proto_item_add_subtree(svcb_param_ti, hf->ett.dns_svcb);

    proto_tree_add_item_ret_uint(svcb_param_tree, hf->hf.dns_svcb_param_key, tvb, cur_offset, 2, ENC_BIG_ENDIAN, &svc_param_key);
    cur_offset += 2;

    proto_tree_add_item_ret_uint(svcb_param_tree, hf->hf.dns_svcb_param_length, tvb, cur_offset, 2, ENC_BIG_ENDIAN, &svc_param_length);
    cur_offset += 2;
    ...
    switch(svc_param_key) {
      case DNS_SVCB_KEY_MANDATORY:
        for (svc_param_offset = 0; svc_param_offset < svc_param_length; svc_param_offset += 2) {
          uint32_t key;
          proto_tree_add_item_ret_uint(svcb_param_tree, hf->hf.dns_svcb_param_mandatory_key, tvb, cur_offset, 2, ENC_BIG_ENDIAN, &key);
      ...
      case DNS_SVCB_KEY_ALPN:
        for (svc_param_offset = 0; svc_param_offset < svc_param_length; ) {
          const uint8_t *alpn;
          proto_tree_add_item_ret_uint(svcb_param_tree, hf->hf.dns_svcb_param_alpn_length, tvb, cur_offset, 1, ENC_BIG_ENDIAN, &svc_param_alpn_length);
          cur_offset += 1;
          proto_tree_add_item_ret_string(svcb_param_tree, hf->hf.dns_svcb_param_alpn, tvb, cur_offset, svc_param_alpn_length, ENC_ASCII|ENC_NA, pinfo->pool, &alpn);
      ...
      case DNS_SVCB_KEY_PORT:
        proto_tree_add_item_ret_uint(svcb_param_tree, hf->hf.dns_svcb_param_port, tvb, cur_offset, 2, ENC_BIG_ENDIAN, &value);
      ...
      case DNS_SVCB_KEY_IPV4HINT:
        for (svc_param_offset = 0; svc_param_offset < svc_param_length; svc_param_offset += 4) {
          proto_tree_add_item(svcb_param_tree, hf->hf.dns_svcb_param_ipv4hint_ip, tvb, cur_offset, 4, ENC_BIG_ENDIAN);
      ...
      case DNS_SVCB_KEY_IPV6HINT:
        for (svc_param_offset = 0; svc_param_offset < svc_param_length; svc_param_offset += 16) {
          proto_tree_add_item(svcb_param_tree, hf->hf.dns_svcb_param_ipv6hint_ip, tvb, cur_offset, 16, ENC_NA);
      ...
      case DNS_SVCB_KEY_DOHPATH:
        proto_tree_add_item_ret_string(svcb_param_tree, hf->hf.dns_svcb_param_dohpath, tvb, cur_offset, svc_param_length, ENC_UTF_8|ENC_NA, pinfo->pool, &dohpath);
      ...
      case DNS_SVCB_KEY_ODOHCONFIG:
        dissect_dns_svcparam_base64(svcb_param_tree, pinfo, svcb_param_ti, hf->hf.dns_svcb_param_odohconfig, tvb, cur_offset, svc_param_length);
        cur_offset += svc_param_length;
        break;
      default:
        if (svc_param_length > 0) {
          proto_tree_add_item(svcb_param_tree, hf->hf.dns_svcb_param_value, tvb, cur_offset, svc_param_length, ENC_NA);
  ...
  switch (dns_type) {
    ...
    case T_HTTPS: /* Service binding and parameter specification (65) */
    {
      ...
      if (data_len > cur_offset - start_offset) {
        cur_offset += dns_dissect_svcb_params(&dissect_dns_svcb_params_hf, tvb, pinfo, rr_tree,
                                              cur_offset, start_offset + data_len);
      }
    }
    break;
...
proto_register_dns(void)
{
  static hf_register_info hf[] = {
    ...
    DNS_SVCB_PARAMS_COMMON_HF_LIST(dissect_dns_svcb_params_hf, "dns.svcb"),
  ...
  static int *ett[] = {
    ...
    &ett_dns_dso,
    &ett_dns_dso_tlv,
    DNS_SVCB_PARAMS_COMMON_ETT_LIST(dissect_dns_svcb_params_hf),
    &ett_dns_extraneous,
    &ett_dns_dnscrypt
  };
