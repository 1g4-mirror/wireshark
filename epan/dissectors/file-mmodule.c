...
static int hf_tag, hf_ftag, hf_mod, hf_mainv, hf_subv, hf_off, hf_tstamp;
static int hf_vtype, hf_vcode0, hf_vcode1, hf_vcode2;
static int hf_tot, hf_nentries, hf_ood, hf_cksm, hf_fna;
static int hf_len, hf_elen;
static int hf_mentry, hf_mcontent;
...
/* prototypes */
static int dissect_m_module(tvbuff_t *tvb, packet_info *pinfo,
...
bool verify_checksum(tvbuff_t *tvb) //, packet_info *pinfo)
{
    ...
    for (; i<(file_len/4) ; i++ ) {
        if (i == 11) {
                //currsum = buf[i];
                currsum = tvb_get_uint32(tvb,44,ENC_LITTLE_ENDIAN);
        }
        else{
                //checksum += buf[i];
                checksum += tvb_get_uint32(tvb,i*4,ENC_LITTLE_ENDIAN);
        }
    }
    checksum = -checksum;
...
bool verify_len(tvbuff_t *tvb)
{
    uint32_t file_len = (uint32_t)tvb_captured_length(tvb);
    uint32_t reported_len = tvb_get_uint32(tvb,0x20,ENC_LITTLE_ENDIAN);
...
static bool dissect_m_module_heur(tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree, void *data)
{
    ...
    if (verify_checksum(tvb) == false)
        return false;
    //if (tvb_get_letohl(tvb,0xc0) !=  0x464c457f) { return (FALSE); }
...
void add_mentry_m_module(proto_tree *subtree,tvbuff_t *tvb, packet_info *pinfo, int offset, int entrysz, struct mentry_info * mentry_table, int idx)
{
    ...
    proto_tree_add_item(mentry_hdr, hf_tag,      tvb, offset+ 0x00, 4, ENC_LITTLE_ENDIAN);
    proto_tree_add_item(mentry_hdr, hf_mod,      tvb,  offset+0x04, 4, ENC_LITTLE_ENDIAN);
    proto_tree_add_item(mentry_hdr, hf_len,      tvb,  offset+0x08, 4, ENC_LITTLE_ENDIAN);
    proto_tree_add_item(mentry_hdr, hf_off,      tvb,  offset+0x0c, 4, ENC_LITTLE_ENDIAN);
    ...
    mentry_table[idx].tag   = tvb_get_uint32(tvb,offset+0x00,ENC_LITTLE_ENDIAN);
    mentry_table[idx].start = tvb_get_uint32(tvb,offset+0x0c,ENC_LITTLE_ENDIAN);
    mentry_table[idx].len   = tvb_get_uint32(tvb,offset+0x08,ENC_LITTLE_ENDIAN);
}

void setup_mentry_table_content(proto_tree *subtree, tvbuff_t *tvb , packet_info *pinfo, struct mentry_info *mentry_table, int idx)
{
...
static int dissect_m_module(tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree, void *data _U_)
{
    ...
    proto_tree_add_item(subtree, hf_nentries, tvb, 0x24, 4, ENC_LITTLE_ENDIAN);
    proto_tree_add_item(subtree, hf_ood,      tvb, 0x28, 4, ENC_LITTLE_ENDIAN);
    proto_tree_add_item(subtree, hf_cksm,     tvb, 0x2c, 4, ENC_NA);

    int nentries = tvb_get_uint32(tvb,0x24,ENC_LITTLE_ENDIAN);
    int entsz = tvb_get_uint32(tvb,0x28, ENC_LITTLE_ENDIAN);
