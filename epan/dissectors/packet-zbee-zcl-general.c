...
dissect_zcl_touchlink_network_join_request(tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree, unsigned *offset, struct zcl_touchlink_commissioning_data * commissioning_data)
{
    ...
    *offset += 16;
    ...
    if(key_index == ZBEE_ZCL_TOUCHLINK_KEYID_MASTER || key_index == ZBEE_ZCL_TOUCHLINK_KEYID_CERTIFICATION){
        if(key_index == ZBEE_ZCL_TOUCHLINK_KEYID_MASTER){
            if(!zbee_sec_get_key_from_keyring("Touchlink Master Key", Touchlink_Master_Key)){
                expert_add_info_format(pinfo, it_key, &ei_zbee_zcl_touchlink_no_key, "No key named 'Touchlink Master Key' found, cannot decrypt transport key");   // xyz
            }else{
    ...
    //        tvbuff_t *key_tvb = tvb_new_child_real_data(tvb, network_key, 16, 16);
    //        add_new_data_source(pinfo, key_tvb, "Decrypted Network Key");
    //        proto_tree_add_item(it_key, hf_zbee_zcl_touchlink_network_key, key_tvb, 0, 16, ENC_NA);
    //        proto_tree_add_item(tree, hf_zbee_zcl_touchlink_network_key, key_tvb, 0, 16, ENC_NA);
        ...
        expert_add_info_format(pinfo, it_key, &ei_zbee_zcl_touchlink_no_key, "Unsupported key %d, cannot decrypt transport key", key_index);   // xyz
    }
