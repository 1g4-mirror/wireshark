...
static int hf_omron_relay_error;
static int hf_omron_pc_fatal_error;
static int hf_omron_pc_non_fatal_error;
...
static void
add_response_code_items(proto_tree *command_tree, tvbuff_t *tvb, unsigned offset) {
    proto_tree_add_item(command_tree, hf_omron_response_code, tvb, offset, 2, ENC_BIG_ENDIAN);
    proto_tree_add_item(command_tree, hf_omron_relay_error, tvb, offset, 1, ENC_BIG_ENDIAN);
    proto_tree_add_item(command_tree, hf_omron_pc_fatal_error, tvb, offset + 1, 1, ENC_BIG_ENDIAN);
    proto_tree_add_item(command_tree, hf_omron_pc_non_fatal_error, tvb, offset + 1, 1, ENC_BIG_ENDIAN);
}
    ...
    if (omron_tree) { /* we are being asked for details */
        ...
        case 0x0101:
        {
            ...
            if(is_response)
            {
                ...
                {
                    add_response_code_items(command_tree, tvb, offset);
...
proto_register_omron_fins(void)
{
    ...
    static hf_register_info hf[] = {
        ...
        { &hf_omron_relay_error,
        { "Relay Error", "omron.response.relayError", FT_BOOLEAN, 8, NULL,  0x80, NULL, HFILL }},

        { &hf_omron_pc_fatal_error,
        { "PC Fatal Error", "omron.response.fatalError", FT_BOOLEAN, 8, NULL,  0x80, NULL, HFILL }},

        { &hf_omron_pc_non_fatal_error,
        { "PC Non-fatal Error", "omron.response.nonFatalError", FT_BOOLEAN, 8, NULL,  0x40, NULL, HFILL }},
