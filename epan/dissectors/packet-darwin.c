...
#define PINFO_DARWIN_MD_HAS_DPEB_ID     1
    uint32_t dpeb_id;             /**< DPEB block id */
#define PINFO_DARWIN_MD_HAS_EDPEB_ID    2
    uint32_t effective_dpeb_id;   /**< Effective DPEB block id */
...
static int
dissect_darwin_dpeb_id(tvbuff_t* tvb _U_, packet_info* pinfo, proto_tree* tree _U_, void* data)
{
    wtap_optval_t* optval = (wtap_optval_t*)data;

    struct darwin_md* darwin = get_darwin_proto_data(pinfo);
    darwin->dpeb_id = optval->uint32val;
    darwin->present_opts |= PINFO_DARWIN_MD_HAS_DPEB_ID;

    return 1;
}

static int
dissect_darwin_effective_dpeb_id(tvbuff_t* tvb _U_, packet_info* pinfo, proto_tree* tree _U_, void* data)
{
    wtap_optval_t* optval = (wtap_optval_t*)data;

    struct darwin_md* darwin = get_darwin_proto_data(pinfo);
    darwin->effective_dpeb_id = optval->uint32val;
    darwin->present_opts |= PINFO_DARWIN_MD_HAS_EDPEB_ID;
...
dissect_darwin_data(tvbuff_t* tvb, packet_info* pinfo, proto_tree* tree, void* data _U_)
{
    ...
    if (darwin->present_opts & (PINFO_DARWIN_MD_HAS_DPEB_ID | PINFO_DARWIN_MD_HAS_EDPEB_ID)) {
        uint32_t proc_id;
        const char* proc_name;
        uint32_t eproc_id;
        const char* eproc_name;

        proc_id = epan_get_process_id(pinfo->epan, darwin->dpeb_id, section_number);
        proc_name = epan_get_process_name(pinfo->epan, darwin->dpeb_id, section_number);

        /* If the effective dpeb id is not present, or is equal to the primary dpeb id,
         * set the eproc_id to proc_id.
         */
        if (((darwin->present_opts & PINFO_DARWIN_MD_HAS_EDPEB_ID) == 0) ||
            (darwin->dpeb_id == darwin->effective_dpeb_id)) {
            eproc_id = proc_id;
            eproc_name = proc_name;
        }
        else {
            eproc_id = epan_get_process_id(pinfo->epan, darwin->effective_dpeb_id, section_number);
            eproc_name = epan_get_process_name(pinfo->epan, darwin->effective_dpeb_id, section_number);
        }
...
proto_reg_handoff_darwin(void)
{
    ...
    dissector_add_uint("pcapng_block_option", OPT_PKT_DARWIN_DPEB_ID, create_dissector_handle(dissect_darwin_dpeb_id, proto_darwin));
    dissector_add_uint("pcapng_block_option", OPT_PKT_DARWIN_EFFECTIVE_DPEB_ID, create_dissector_handle(dissect_darwin_effective_dpeb_id, proto_darwin));
    dissector_add_uint("pcapng_block_option", OPT_PKT_DARWIN_SVC_CODE, create_dissector_handle(dissect_darwin_svc_code, proto_darwin));
    dissector_add_uint("pcapng_block_option", OPT_PKT_DARWIN_MD_FLAGS, create_dissector_handle(dissect_darwin_md_flags, proto_darwin));
    dissector_add_uint("pcapng_block_option", OPT_PKT_DARWIN_FLOW_ID, create_dissector_handle(dissect_darwin_flow_id, proto_darwin));
    dissector_add_uint("pcapng_block_option", OPT_PKT_DARWIN_TRACE_TAG, create_dissector_handle(dissect_darwin_trace_tag, proto_darwin));
    dissector_add_uint("pcapng_block_option", OPT_PKT_DARWIN_DROP_REASON, create_dissector_handle(dissect_darwin_drop_reason, proto_darwin));
    dissector_add_uint("pcapng_block_option", OPT_PKT_DARWIN_DROP_LINE, create_dissector_handle(dissect_darwin_drop_line, proto_darwin));
    dissector_add_uint("pcapng_block_option", OPT_PKT_DARWIN_DROP_FUNC, create_dissector_handle(dissect_darwin_drop_func, proto_darwin));
    dissector_add_uint("pcapng_block_option", OPT_PKT_DARWIN_COMP_GENCNT, create_dissector_handle(dissect_darwin_comp_gencnt, proto_darwin));
}
