        ...
        if (!session) {
            ...
            if (gtpv2_hdr->message == GTPV2_CREATE_SESSION_REQUEST){
                ...
                if(is_deinterlacing_supported(pinfo)) {
                    ...
                    if ((get_gtp_session_frame(pinfo->dst, (uint32_t)gtpv2_hdr->teid, conversation->conv_index, &frame_teid_cp) == 1)) {
...
dissect_gtpv2(tvbuff_t * tvb, packet_info * pinfo, proto_tree * tree, void* data _U_)
{
    ...
    */
    ...
    if (args && !PINFO_FD_VISITED(pinfo)) {
        ...
        if(is_deinterlacing_supported(pinfo)) {
            fill_map(args->teid_list, args->ip_list, pinfo->num, conversation->conv_index);
        }
        else {
            // unhandled.. but unexpected
        }
