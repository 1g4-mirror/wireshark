...
+
--
Inhibit generating to __outfile__ the capture comment "File created by merging:...".

By default a multiline capture comment is generated to __outfile__ that
lists all __infile__ arguments. Each __infile__ is listed on its own line
prefixed with the text "File%d: %s" where %d is the ordinal number of the
__infile__ argument and %s is the corresponding __infile__ argument. If
the generated capture comment is longer than 65535 bytes it is silently
dropped from __outfile__.
--
