...
+
--
Use the specified configuration profile with -C <profile>. By default, TShark loads profiles from the user's personal configuration directory (e.g., ~/.config/wireshark/profiles/ on Linux).

If --global-profile is specified, the profile will instead be loaded from the system-wide global configuration directory (typically /usr/share/wireshark/profiles/).

This is particularly useful in automated environments, shared systems, or testing setups where relying on user-specific profiles is not desired or reproducible.
--
