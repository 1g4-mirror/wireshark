                         ...
                         int *err, char **err_info)
{
     ...
     *
     * Note that in those cases ts / iface_info.time_units_per_second can be no
     * larger than 10^9, which means that without tsoffset they can't specify a
     * time past 2001-09-09T01:46:46, which is why we must reduce precision when
     * writing if we don't write tsoffset.
     *
     ...
     * If __SIZEOF_INT128__ is defined we can use unsigned __int128.
     * (There's __uint128_t on even older gcc, clang, ICC, but harder to test
     * without doing so in CMake.) It might be just faster to check overflow
     * and handle our two special cases.
     */
