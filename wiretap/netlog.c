...
/* Note that this chokes on large files. A 110 MB file took about 6 minutes to load. Adjust as appropriate. */
...
/*
Represents a TCP or UDP session.
...
/* Represents the byte offset of a single JSON object which can be parsed to obtain the associated data*/
typedef struct {
    uint32_t offset;
    uint32_t length;
...
static bool parse_log_event_constants(char *filebuf, jsmntok_t *root_json_token, NetLogEventConstants *out)
{
    ...
    ws_debug("TCP_CONNECT: %li", out->TCP_CONNECT);
    ws_debug("SOCKET_BYTES_RECEIVED: %li", out->SOCKET_BYTES_RECEIVED);
    ws_debug("SOCKET_BYTES_SENT: %li", out->SOCKET_BYTES_SENT);
    ws_debug("SOCKET_CLOSED: %li", out->SOCKET_CLOSED);
    ws_debug("SSL_SOCKET_BYTES_RECEIVED: %li", out->SSL_SOCKET_BYTES_RECEIVED);
    ws_debug("SSL_SOCKET_BYTES_SENT: %li", out->SSL_SOCKET_BYTES_SENT);
    ws_debug("UDP_BYTES_RECEIVED: %li", out->UDP_BYTES_RECEIVED);
    ws_debug("UDP_BYTES_SENT: %li", out->UDP_BYTES_SENT);
    ws_debug("UDP_CONNECT: %li", out->UDP_CONNECT);
    ws_debug("UDP_LOCAL_ADDRESS: %li", out->UDP_LOCAL_ADDRESS);
...
static bool generate_packet(const wtap* wth, wtap_rec* rec, const TransportSession* session, const uint8_t* payload, const size_t payload_len)
{
    ...
    rec->ts.nsecs = (int)((session->timestamp % 1000) * 1000 * 1000); //1755205115523
    ...
    if (session->ip_version == IP_VERSION_4) {
        ...
        *p++ = 0x00; // DSCP/ECN
        uint16_t total_len = packet_size;
        *(uint16_t*)p = g_htons(total_len); p += 2;
        *(uint16_t*)p = g_htons(0x1234); p += 2; // Identification
    ...
    if (session->transport == TransportProtocol_TCP) {
        ...
        *p++ = 0x10;    // TCP flags (e.g. SYN=0x02, ACK=0x10, FIN=0x01)
 ...
 */
static bool netlog_read_packet(const wtap* wth, wtap_rec* rec, GHashTable *json_packets_ht, const int index, int* err, char **err_info, FILE_T fh)
{
    JSONPacket* json_packet = g_hash_table_lookup(json_packets_ht, GINT_TO_POINTER(index));
    if (!json_packet){
        return false;
    }
...
/*
 * Given an input string of base64 data, returns the length of the decoded data.
 */
size_t base64_decoded_length(const char *base64_input) {
    if (!base64_input) return 0;
    ...
    if (input_len >= 2) {
        if (base64_input[input_len - 1] == '=') padding++;
        if (base64_input[input_len - 2] == '=') padding++;
    } else if (input_len >= 1 && base64_input[input_len - 1] == '=') {
        padding++;
    }

    /* Each 4 base64 chars represent 3 bytes of data */
...
JSONPacket* handle_traffic_event(char* filebuf, jsmntok_t* event_entry, GHashTable* sessions_table, int64_t event_id, TrafficDirection direction){
    ...
    if (session->transport == TransportProtocol_TCP) {
        ...
        const gsize payload_len = base64_decoded_length(base64_bytes);
...
static bool parse_json_events(char* filebuf, const NetLogEventConstants netlog_event_constants, jsmntok_t* json_events, GHashTable *json_packets_ht)
{
    ...
    for (int i = 0; i < json_array_len && (event_entry = json_get_next_object(event_entry)); i++) {
        ...
        if (json_get_int(filebuf, event_entry, "type", &type_val)){
            ws_debug("Processing event %d type: %li", i, type_val);
        } else {
...
static bool netlog_read(wtap* wth, wtap_rec* rec, int* err, char** err_info, int64_t* data_offset)
{
    ...
    if (!netlog_read_packet(wth, rec, netlog_state->json_packets_ht, netlog_state->index, err, err_info, wth->fh)) {
        return false;
    }

    *data_offset = netlog_state->index;
    netlog_state->index += 1;
...
{
    /* Release the requested packet at a time: */
    NetLogState* netlog_state = wth->priv;
